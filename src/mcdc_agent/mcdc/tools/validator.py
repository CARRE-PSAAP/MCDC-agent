...
class ValidationResult:
    ...
    lost_particle_coords: List[tuple] = field(default_factory=list)  # (x, y, z) coords
...
"""
    ...
    def validate(self, script: str, require_run: bool = True) -> ValidationResult:
        """
        Execute script with dry-run settings and analyze output.

        Args:
            script: Complete MCDC Python script as string
            require_run: Whether to require a successful simulation run (default True)

        Returns:
            ValidationResult with status, runtime report or error message
        """
        # Inject dry-run settings before mcdc.run()
        modified_script = self._inject_dry_run_settings(script, force_run=require_run)
        ...
        try:
            ...
            return self._analyze_output(output, result.returncode, require_run=require_run)
            ...
            if not require_run and not error_msg and not warnings:
                return ValidationResult(
                    status=ValidationStatus.SUCCESS,
                    runtime_report=f"Syntax valid (timed out at {self.timeout}s, no errors)",
            ...
            if simulation_running:
                ...
                if error_msg:
                    diag_msg = (
                        f"Simulation started ({mode}) but failed before completion.\n"
                ...
                if warnings:
                    return ValidationResult(
                        ...
                        runtime_report=(
                            f"Simulation running ({mode}, timed out at {self.timeout}s "
                            "with warnings detected)"
                        ),
                        warnings=warnings,
                        raw_output=partial_output,
                    )
            ...
            return ValidationResult(
                status=ValidationStatus.ERROR if error_msg else ValidationStatus.TIMEOUT,
                error_message=diag_msg,
                warnings=warnings,
                raw_output=partial_output,
            )
    ...
    def _inject_dry_run_settings(self, script: str, force_run: bool = True) -> str:
    ...
    def _analyze_output(self, output: str, returncode: int, require_run: bool = True) -> ValidationResult:
        """
        ...
        if not require_run and not error_msg and returncode == 0:
            return ValidationResult(
                status=ValidationStatus.SUCCESS,
                runtime_report="Syntax valid (no run required).",
                warnings=warnings,
                raw_output=output,
            )
        ...
        elif require_run and not error_msg:
    ...
    def _extract_error(self, output: str) -> Optional[str]:
        """Extract the most relevant error message from output, including traceback."""
        # Look for full Python traceback first (most useful for debugging)
        traceback_pattern = re.compile(
            r'(Traceback \(most recent call last\):.*?(?:\w+Error|\w+Exception): .+?)(?:\n\n|\Z)',
            re.DOTALL
        )
        traceback_match = traceback_pattern.search(output)
        if traceback_match:
            # Return full traceback (limited to reasonable size)
            return traceback_match.group(1)[:2000]
        
        # Look for individual error lines
        error_patterns = [
            # MCDC-specific errors
            re.compile(r"(MCDC Error: .+?)(?:\n|$)", re.IGNORECASE | re.MULTILINE),
            # Explicit console error lines
            re.compile(r"(^ERROR:\s+.+?)(?:\n|$)", re.IGNORECASE | re.MULTILINE),
            # Traceback with specific error (fallback if full pattern didn't match)
    ...
    def validate_numba(self, script: str, timeout: float = 30.0) -> ValidationResult:
        """
        Validate script in numba mode to get detailed particle loss coordinates.
        
        Numba mode provides (x, y, z) coordinates for lost particles, enabling
        better geometry diagnosis. Use this after Python mode validation passes
        but geometry issues are detected.
        
        Args:
            script: Complete MCDC Python script as string
            timeout: Longer timeout for JIT compilation (default 30s)
            
        Returns:
            ValidationResult with lost_particle_coords populated if particles were lost
        """
        # Inject dry-run settings before mcdc.run()
        modified_script = self._inject_dry_run_settings(script)

        # Write to temp file
        temp_dir = tempfile.mkdtemp(prefix="mcdc_validate_numba_")
        script_path = Path(temp_dir) / "dry_run_script.py"
        script_path.write_text(modified_script)
        ...
        try:
            ...
            # Run in numba mode to get coordinate output
            result = subprocess.run(
                [sys.executable, str(script_path), "--mode=numba"],
                capture_output=True,
                text=True,
                timeout=timeout,
                cwd=temp_dir,
            ...
            # Parse lost particle coordinates
            coords = self._parse_lost_coordinates(output)
            
            # Analyze output (reuse existing logic)
            validation_result = self._analyze_output(output, result.returncode)
            validation_result.lost_particle_coords = coords
            ...
            partial_output = ""
            if e.stdout:
                partial_output = e.stdout.decode() if isinstance(e.stdout, bytes) else e.stdout
            if e.stderr:
                err = e.stderr.decode() if isinstance(e.stderr, bytes) else e.stderr
                partial_output += "\n" + err

            coords = self._parse_lost_coordinates(partial_output)
            warnings = self._extract_warnings(partial_output)
            
            # Check if simulation was running (has progress/k-eff output)
            progress_pattern = re.compile(r"\[\=*\s*\]\s*\d+%")
            eigenmode_pattern = re.compile(r"^\s*\d+\s+[\d.]+\s+", re.MULTILINE)
            simulation_running = progress_pattern.search(partial_output) or eigenmode_pattern.search(partial_output)
            
            if simulation_running and coords:
                # Got coordinates, geometry diagnosis possible
                return ValidationResult(
                    status=ValidationStatus.WARNING,
                    error_message="Particles lost during simulation",
                    warnings=warnings,
                    raw_output=partial_output,
                    lost_particle_coords=coords,
                )
            elif simulation_running and not coords and not warnings:
                # Running fine, just slow (JIT compilation)
                return ValidationResult(
                    status=ValidationStatus.SUCCESS,
                    runtime_report=f"Simulation running (timed out at {timeout}s, no errors)",
                    raw_output=partial_output,
                )
            else:
                return ValidationResult(
                    status=ValidationStatus.TIMEOUT,
                    error_message=f"Numba mode timed out after {timeout}s",
                    warnings=warnings,
                    raw_output=partial_output,
                    lost_particle_coords=coords,
                )
            ...
            return ValidationResult(
                status=ValidationStatus.ERROR,
                error_message=f"Numba validation failed: {str(e)}",
            )
        finally:
            if self.cleanup:
                self._cleanup_temp(temp_dir)

    def _parse_lost_coordinates(self, output: str) -> List[tuple]:
        """
        Extract (x, y, z) coordinates from 'A particle is lost at (x y z t)' lines.
        
        Args:
            output: Raw subprocess output containing MCDC messages
            
        Returns:
            List of (x, y, z) tuples for each lost particle (max 20 for efficiency)
        """
        pattern = r'particle is lost at \( ([-\d.]+) ([-\d.]+) ([-\d.]+)'
        matches = re.findall(pattern, output, re.IGNORECASE)
        
        # Convert to float tuples, limit to 20 for efficiency
        coords = []
        for m in matches[:20]:
            try:
                coords.append((float(m[0]), float(m[1]), float(m[2])))
            except ValueError:
                continue
        return coords
